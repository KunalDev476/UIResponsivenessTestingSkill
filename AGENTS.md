...
- Agent monitor instructions live inside the packaged skill under `ui-responsive-testing-skill/agents/`.

## UI Responsive QA

- Use the `ui-responsive-testing-skill` skill only when the user explicitly requests it by name or asks for responsive UI QA generation.
- The canonical skill is `ui-responsive-testing-skill/SKILL.md`.
- After generating or updating a responsive QA spec, use the `qa-visual-review-monitor` monitor agent defined in `ui-responsive-testing-skill/agents/qa-visual-review-monitor.toml` to process pending AI review hooks.
